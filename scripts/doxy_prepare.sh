...
# Netx, SAA accessors
tmp=$(echo "$tmp" | sed -E 's/^[ \t]*COSM_HAL_SENSOR_ACCESSOR\((.*),(.*),(.*)\)/COSM_HAL_SAA_ACCESSOR(sensor,\1,\2,\3)/g')
tmp=$(echo "$tmp" | sed -E 's/^[ \t]*COSM_HAL_SENSOR_ACCESSOR\((.*),(.*)\)/COSM_HAL_SAA_ACCESSOR(sensor,\1,\2)/g')

tmp=$(echo "$tmp" | sed -E 's/^[ \t]*COSM_HAL_ACTUATOR_ACCESSOR\((.*),(.*),(.*)\)/COSM_HAL_SAA_ACCESSOR(actuator,\1,\2,\3)/g')
tmp=$(echo "$tmp" | sed -E 's/^[ \t]*COSM_HAL_SAA_ACCESSOR\((.*),(.*),(.*),(.*)\)/  \4 \2* \3(void) \4 {return \1<\2>();}/g')
tmp=$(echo "$tmp" | sed -E 's/^[ \t]*COSM_HAL_SAA_ACCESSOR\((.*),(.*),(.*)\)/  \2* \3(void) {return \1<\2>();}/g')
