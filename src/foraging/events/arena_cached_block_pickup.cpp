/**
 * \file arena_cached_block_pickup.cpp
 *
 ...
 * Member Functions
 ******************************************************************************/
void arena_cached_block_pickup::visit(cfsm::cell2D_fsm& fsm) {
  fsm.event_block_pickup();
} /* visit() */

void arena_cached_block_pickup::visit(cds::cell2D& cell) {
...
void arena_cached_block_pickup::visit(cfds::arena_map& map) {
  ER_ASSERT(m_real_cache->n_blocks() >= base_cache::kMinBlocks,
            "< %zu blocks in cache",
            base_cache::kMinBlocks);
