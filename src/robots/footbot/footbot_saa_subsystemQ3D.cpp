/**
 * \file footbot_saa_subsystemQ3D.cpp
 *
 ...
 ******************************************************************************/
footbot_saa_subsystemQ3D::footbot_saa_subsystemQ3D(
 ...
 * Member Functions
 ******************************************************************************/
void footbot_saa_subsystemQ3D::steer_force2D_apply(void) {
...
rmath::vector2d footbot_saa_subsystemQ3D::linear_velocity(void) const {
...
double footbot_saa_subsystemQ3D::angular_velocity(void) const {
  auto reading = sensing()->diff_drive()->reading();

  return (reading.vel_right - reading.vel_left) / reading.axle_length;
} /* angular_velocity() */

double footbot_saa_subsystemQ3D::max_speed(void) const {
  return actuation()->governed_diff_drive()->max_speed();
} /* max_speed() */

rmath::vector2d footbot_saa_subsystemQ3D::position(void) const {
  /*
   * When projecting onto the plane defined by the robot's current inclination
   * angle, (X,Y) position is the same as for the XY plane, so we project on
   * that.
   */
  return sensing()->position().project_on_xy();
} /* position() */
