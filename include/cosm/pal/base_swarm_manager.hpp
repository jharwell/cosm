...
class base_swarm_manager : public rer::client<base_swarm_manager> {
 ...
 protected:
  ...
  /**
   * \brief Push a string that will be prepended to all logged messages until
   * the next pop onto the NDC stack.
   */
  virtual void ndc_push(void) const = 0;

  /**
   * \brief Pop the last NDC off the stack.
   */
  virtual void ndc_pop(void) const = 0;
