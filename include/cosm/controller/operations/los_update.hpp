...
#ifndef INCLUDE_COSM_CONTROLLER_OPERATIONS_LOS_UPDATE_HPP_
#define INCLUDE_COSM_CONTROLLER_OPERATIONS_LOS_UPDATE_HPP_
...
class los_builder {
 public:
  ...
  los_builder(const los_builder&) = delete;
  los_builder& operator=(const los_builder&) = delete;
  los_builder(los_builder&&) = delete;
  los_builder& operator=(los_builder&&) = delete;
};
...
void los_set(TController* const controller,
                   const TGrid* const grid,
                   size_t los_grid_units) {
  auto builder = los_builder();
...
template <typename TController,
          typename TSrcGrid,
          typename TLOS>
class los_update final
    : public rer::client<los_update<TController,
                                          TSrcGrid,
                                          TLOS>>,
      private boost::static_visitor<void> {
 public:
  explicit los_update(TSrcGrid* const grid)
      : ER_CLIENT_INIT("cosm.support.los_update"), mc_grid(grid) {}
   ...
   */
  los_update(const los_update&) = default;
  los_update& operator=(const los_update&) = delete;
  ...
  void operator()(TController* const controller) const {
    ...
    detail::los_set<TController, TLOS>(controller,
                                             mc_grid,
                                             los_grid_size);
  }
...
#endif /* INCLUDE_COSM_CONTROLLER_OPERATIONS_LOS_UPDATE_HPP_ */
